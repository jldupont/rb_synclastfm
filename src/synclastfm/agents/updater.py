...
class Updater(gobject.GObject): #@UndefinedVariable
    ...
    def h_track(self, _, track):
        ...
        try:    rbid=long(mbe.rbid)
            ...
            print "rbid is not an integer!"
            return True
        
        try:
            dbe=self._db.entry_lookup_by_id(rbid)
        except Exception,e:
            print "EXCEPTION: %s" % e
            return True
        
        if dbe is None:
            print "NOT FOUND: id(%s)" % rbid
            return True
        ...
        try:    l=len(track_mbid)
        except: l=0
        
        ## If we don't have a UUID for this track,
        ##  then there ain't much we can do...
        if l==0:
            print "*** track_mbid not set"
            return True
        
        title=self._db.entry_get(dbe, rhythmdb.PROP_TITLE)
        print "title: %s" % title
        
        #PROP_COPYRIGHT
        #PROP_MUSICBRAINZ_TRACKID
